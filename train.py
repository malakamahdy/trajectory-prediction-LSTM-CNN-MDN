...
def train_model(num_epochs=20, batch_size=32, history=8, future=12,
                device=None, load_existing=False, model_path=MODEL_PATH):
    ...
    if load_existing and os.path.exists(model_path):
        ...
        print(f"{YELLOW}Starting NEW training session (weights initialized randomly).{RESET}")
    print()
    ...
    for epoch in range(num_epochs):
        ...
        avg_val = total_val / len(val_dl) if len(val_dl) else float("nan")
